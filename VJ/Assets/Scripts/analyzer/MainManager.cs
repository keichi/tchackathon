...
namespace TCHackathon
{
	public class MainManager : MonoBehaviour
	{
		...
		void Start()
		{
			...
			AudioPath = Application.streamingAssetsPath + "/Sounds/gracenote.json";
			AnalyzeFile(AudioPath);
		...
		void Update()
		{
			...
			{
				currentTime = 0.0f;
				int audioTime = (int)(UnityEngine.Mathf.Clamp(audioManager.audioTime, 0.0f, (float)(Moods.Length - 1.0f)));
				vJSequencer.ChangeMood(Moods[audioTime]);
				if (debugConsole)
				{
					debugConsole.DebugText = Moods[audioTime] + "\nAudioTime:" + audioManager.audioTime + "/" + audioManager.audioMaxTime;
				}
			}
			...
			if (nextBeat >= BeatTimings.Length)
			{
				nextBeat = 0;
			}
		}
		...
		private void AnalyzeJson(string s)
		{
			var json = (IDictionary)Json.Deserialize(s);
			var features = (IDictionary)json["features"];
			var beats = (IList)features["BEATS"];
